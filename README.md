...
> **Student:** Yuxiang Lai (update contact info before submission)  
> **Prompt:** HW 1 from *Lecture Notes on Model-Based Machine Learning – Homework*  
> **Artifacts:** `HW11.ipynb`, exported figures (`SIR_Dynamics.png`, `SEIR_birth_death_365.png`, `SEIR_birth_death_1200.png`, `heatmap.png`)

This README is organized exactly like the homework prompt (parts A–E) so the grader can verify each requirement without opening the notebook.
...
### Discussion points
1. **Wave patterns:** The long run displays a repeating boom–bust cycle in I(t); wave amplitude decreases slowly as μ removes individuals from all compartments.
2. **Effect of exposed compartment:** σ introduces a latent period, smoothing the infection curve and providing time for interventions before infectious peaks.
3. **Impact of μ:** Births counterbalance immunity buildup, making it impossible to eradicate the disease without additional interventions (vaccination, waning immunity modeling, etc.).
...
- **Novelty (bonus):** Added interpretable β–γ heat maps and long-horizon endemic analysis to go beyond the minimum spec.
